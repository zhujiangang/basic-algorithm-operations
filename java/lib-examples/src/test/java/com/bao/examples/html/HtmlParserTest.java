...
public class HtmlParserTest extends TestCase
{
		...
		try
		{
			...
			parser
				.setInputHTML("<head><title>OrFilter Test</title>"
					...
					+ "<select><option id='1'>1</option><option id='2'>2</option><option id='3'>all</option></select>"
	...
	public void testTable()
	{
		...
		myParser = Parser.createParser("<body> " + "<table id='table1' >"
			+ "<tr><td>1-11</td><td>1-12</td><td>1-13</td></tr>"
			+ "<tr><td>1-21</td><td>1-22</td><td>1-23</td>   </TR>"
	...
	public void testVisitorAll()
	{
		...
		{
			Parser parser = new Parser();
			parser.setURL("http://www.yahoo.com");
			parser.setEncoding(parser.getEncoding());
			NodeVisitor visitor = new NodeVisitor()
			{
				public void visitTag(Tag tag)
				{
					logger.fatal("testVisitorAll()  Tag name=" + tag.getTagName() + ", Class="
						+ tag.getClass());
				}
			};
	...
	{
		String inputHTML = "<html>" + "<head>" + "<title>Welcome to the HTMLParser website</title>"
			+ "</head>" + "<body>" + "Welcome to HTMLParser" + "<table id='table1' >"
